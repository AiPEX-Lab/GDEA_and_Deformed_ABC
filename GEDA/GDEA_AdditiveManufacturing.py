...
config = {
    "model": {
        ...
        "deq_history_size": 5 # AA가 얼마나 많은 과거 계산을 참조할지
    },
    "data": {
...
class AdditiveManufacturing(Dataset):
    def __init__(self, root, mode="train", k_list=[8, 16, 32], transform=None):
    ...
    def __getitem__(self, idx):
        ...
        disp = torch.tensor(data["disp"], dtype=torch.float32)  # (N, 3)
        elems = torch.tensor(data["elems"], dtype=torch.long)   # (M, 8)
    ...
    def elem_edge_index(self, elems):
        ...
        edge_pairs = elems[:, local_edges].reshape(-1, 2)
        edge_pairs, _ = torch.sort(edge_pairs, dim=1)

        encoded = edge_pairs[:, 0] * num_nodes + edge_pairs[:, 1]
...
class GPTConfig():
    ...
    def __init__(self,attn_type='linear', embd_pdrop=0.0, resid_pdrop=0.0,attn_pdrop=0.0, 
                 ...
                 input=3, mlp_layers=4,deq_epsilon=0.01,deq_history_size=5):
...
class GDEA(nn.Module):
    def __init__(self,
        ...
        self.project_in = MLP(self.input, n_hidden, n_hidden, n_layers=mlp_layers, act=act)
...
def AdditiveManufacturing_procedure(args):
    ...
    if args['log']['wandb']:
        import wandb
        wandb.init(
            project=args['log']['wandb_project'],
            name=args['saving_name'],
            tags=[args['model']['model_name'], args['data']['dataset']],
            config=args
        )
    ...
    train_dataset = AdditiveManufacturing(
    ...
    test_dataset = AdditiveManufacturing(
        root=args['data']['data_path'],
        mode="test",
        transform=None
    )
    ...
    trainer.process(
        ...
        valid_loader=test_loader,  # Validate = test
        optimizer=optimizer,
        scheduler=scheduler,
        criterion=criterion,
        loss_list=['disp']
